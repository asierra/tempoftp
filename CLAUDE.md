...
# Run (real backend, single worker only — background tasks run in-process)
...
- `eliminar_expiradas()` (invoked both by `DELETE /tmpftp/expired` and a background `_cleanup_loop()` in `main.py` firing every `TEMPOFTP_CLEANUP_INTERVAL_HOURS`, default 24h) purges expired `listo` requests: deletes the `/data/{usuario}/{id}` subdir, marks SQLite `expirado`, and drops the MySQL FTP user entirely only if the user has no other active (non-terminal) requests (`obtener_activas_por_usuario`) — so one user's multiple concurrent requests share one FTP account safely.
